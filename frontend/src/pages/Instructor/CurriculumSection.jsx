...
function QuizEditor({ item, onUpdate }) {
  ...
  const setDuration = (minutes) => {
    ...
    onUpdate({ durationSeconds: clamped * 60 });
  };

  const commitDurationText = () => setDuration(parseInt(durationText, 10) || durationMinutes);
    ...
    <div className="space-y-6">
      <div className="space-y-2.5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center rounded-xl border border-border bg-background overflow-hidden shrink-0">
            <button
              type="button"
              onClick={() => setDuration(durationMinutes - 1)}
              disabled={durationMinutes <= 1}
              className="h-10 w-10 flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
            ...
            <input
              ...
              className="w-12 h-10 text-center text-sm font-bold bg-transparent border-x border-border outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
            <button
              type="button"
              onClick={() => setDuration(durationMinutes + 1)}
